<template>
  <div class="app-container" id="wrapper">
    <div class="module-title" style="display: flex;justify-content: space-between">合同管理
    </div>
    <div class="app__content">
      <el-row :gutter="10">
        <el-button type="primary" size="small" @click="toAU()">新建合同</el-button>
      </el-row>
      <div class="search-box">
        ...
        <el-form :inline="true" label-width="70px">
          ...
          <el-form-item label="退租期限">
            <el-select v-model="sm.value"  size="small" placeholder="请选择">
              <el-option
                  v-for="item in [{value:1,label:'1个月内'},{value:3,label:'3个月内'},{value:6,label:'6个月内'},{value:12,label:'1年内'}]"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
              </el-option>
      ...
      <el-table class="mb15"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <label v-if="scope.row">{{scope.row.items[0].quadrature}}</label>  ㎡
          </template>
        </el-table-column>
        <el-table-column
            align="center"
        ...
        <el-table-column
            ...
            >
            <template slot-scope="scope">
              {{scope.row.items[0].goodsPrice}} 元/㎡·天
            </template>
        </el-table-column>
        <el-table-column
            align="center"
...
export default {
  ...
  data() {
    return {
      ...
      sm: {page: 1, count: 20, orderSearchModel:{businessType: 211000000000000000, ptradingAreaId:ConfigUtil.getTradingAreaId(), needItems:1, deleteStatus:0}},
      resetSM:null,
      totalCount: 0,
      pageList: [],
      group: null,
...
//        exportModel.userSearch.userId = ids.join(",");
    ...
    toAU(id) {
      var url = 'contract_list/contract_au';
      if (id) {
        url += '?id=' + id;
      }
