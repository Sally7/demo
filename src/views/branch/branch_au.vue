      ...
      <el-form :model="shop" :rules="rules" ref="shop" label-position="left" class="form-content zj-trisect">
        ...
        <el-form-item class="item-vertical" label="品牌设置">
          ...
          <el-col :span="8" class="ml10">
            <el-button size="small" type="primary" @click="dialogBrandsVisible = true;tempChoosedBrandList=[]">设置品牌</el-button>
          </el-col>
        </el-form-item>
            ...
            <div class="brand-list">
                <ul>
                    <li class="brand-box" v-for="brand in brandList">
                        <div class="brand-item" :class="{selected:brand.isSelect}" @click="brand.isSelect=!brand.isSelect;brandSelect(brand,brand.isSelect)">
  ...
  export default {
    data() {
      return {
        ...
        groupRations: [ //所有商圈
          {id: "402881ae4cd4a25a014cd4f8bbf3000f", type: 1, name: "报贝"}
        ],
        disabled: false,
        timeOptions: {
          start: '00:00',
        ...
        branchGoods: [],
        brandList: [], //所有品牌
        choosedBrandList: [], //选中的品牌
        isBrandEmpty: false, //品牌是否为空
        categoryIdList: [], //当前分类id列表
        tempChoosedBrandList: [],//临时品牌选择list
    ...
    methods: {
        ...
        if(this.id){ //修改分店
          ...
          if(this.shop.groupRationList.length>0){
            this.shop.groupRationList.forEach(function(ration, index){
              that.RationList.push(ration.tradingAreaId);
            })
          }
          //获取品牌
          GoodsService.doSearchByGroupId(this.id).then(res => {
            if(res.status == 1){
              ...
              that.choosedBrandList = res.data;
              that.tempChoosedBrandList = res.data;
            }
          });
          ...
          GoodsService.doSearchCategoryByGroupId(this.id).then(res => {
            if(res.status == 1) {
              //console.log(res.data);
              res.data.forEach(function(item, index){
                that.categoryIdList.push(item.id);
              })
            }
          });
        }else { //添加分店
          //var res = await GroupService.doGetOneGroupById(this.shop.generalId);
          //console.log(res.data);
        }
      },
      //获取商品分类
      ...
      //选择品牌
      brandSelect(brand, checked){
        console.log(this.tempChoosedBrandList);
        if(checked){//选中
          this.tempChoosedBrandList.push(brand);
        }else {//取消
          //删除临时品牌
          for(var i=0; i<this.tempChoosedBrandList.length; i++){
            if(brand.id === this.tempChoosedBrandList[i].id){
              this.tempChoosedBrandList.splice(i,1);
            }
          }
        }
        //console.log(this.tempChoosedBrandList);
      ...
      doConfirm(){
        ...
        this.tempChoosedBrandList.forEach(function(item, index){
