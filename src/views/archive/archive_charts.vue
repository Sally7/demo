  ...
  <div class="app-container" id="wrapper" >
    ...
    <div class="chart-box">
      ...
      <div class="chart-box-item">
        <bar-line-chart v-if="armyYear.armyYearList.length>0" :chartList="armyYear.armyYearList" :dataList="armyYear.dataList" title="time" chartName="服役时间统计"></bar-line-chart>
      </div>
    </div>
    <div class="chart-box">
      <div class="chart-box-item" style="border-right: 10px solid #f0f0f0">
    ...
    <div class="chart-box">
      <div class="chart-box-line-item">
        <pictorial-bar-chart v-if="specialityList.data.length>0" :chartList="specialityList"  title="speciality" chartName="退役军人专业统计"></pictorial-bar-chart>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default{
    ...
    components: {
      pieChart,barChart,barLineChart,roseChart,pictorialBarChart
    },
    created() {
    ...
    methods:{
      ...
      async arrangeStatistics(){
        ...
        if(res.status){
          const list = [{ name: "在岗" , value:0 },
            ...
            { name: "自由职业" , value:0 }]
          res.data.forEach((item,index) => {
            item.name=='在岗'?list[0].value = list[0].value + item.count
            :item.name=='已退休'?list[1].value = list[1].value +item.count
              :item.name=='未安置'?list[2].value = list[2].value + item.count
                :item.name=='安置未上岗'?list[3].value = list[3].value +item.count
                  :item.name=='安置后下岗'?list[4].value = list[4].value + item.count
                    :item.name=='回乡务农'?list[5].value = list[5].value +item.count:list[6].value = list[6].value +item.count
          })
      ...
      async armyYearStatistics(){
        ...
        if(res.status){
          ...
          console.log('army'+  this.armyYear)
        }
      },
      doSearchAreaInfo(){
        this.allAreaStatistics(null,null,this.areaId)
        this.threeGeneraAreaStatistics(null,null,this.areaId)
        this.baoLianAreaStatistics(null,null,this.areaId)
        this.abnormalAreaStatistics(null,null,this.areaId)
        setTimeout(() => {
      ...
      handleStatistics(){
        this.areaList.forEach((item,index) => {
          this.distributeList[index]={
            ...
            normalCount:item.abnormalCount&& item.allAreaCount?item.allAreaCount-item.abnormalCount:0,
