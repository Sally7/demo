...
<template>
  <div class="app-container" id="wrapper">
    <div class="module-title" style="display: flex;justify-content: space-between">装备分类管理
    </div>
    <div class="app__content">
      <el-row :gutter="10">
        <el-button type="primary" size="small" @click="toAU()">新建装备分类</el-button>
      </el-row>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      async doPage(){
        var res = await GoodsService.doSearchTree();
        this.pageList = res.data;
        if(this.pageList.length==0) {
          this.loadingText = '暂无装备分类信息';
        }
      },
      doExport() {
      ...
      renderContent(h, { node, data, store }) {
        ...
        <div>
        <span>
        <el-button size="mini" type="text" on-click={ () => this.toAU(data.id) }>编辑</el-button>
        <el-popover ref="popover" placement="top" width="200" value={this.visible} trigger="click" style="margin:0 6px;">
            <p>确定删除{data.name}吗？</p>
        <div style="text-align: right; margin: 0">
            <el-button size="mini" type="text" on-click={()=>{this.hide()}}>取消</el-button>
        <el-button type="primary" size="mini" on-click={()=>{this.remove(data)}}>确定</el-button>
        </div>
