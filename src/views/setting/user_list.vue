...
<template>
    <div class="app-container" id="wrapper">
        ...
        <div class="app__content">
            <el-button type="primary" v-if="false" @click="userAUDialogVisible = true;cleanForm();doSearch()" style="margin-bottom: 20px">+添加用户</el-button>
            <el-table class="mb15"
...
<script>
  ...
  export default{
    ...
    methods:{
      async doSearch() {
        var that = this;
        var res = await UserService.doPage({searchType:1, groupId:this.groupId});
      ...
      async doDelete(e){
        var res = await UserAdminService.doDeleteUser(e.id);
        if(res.status === 1){
          this.doSearch();
        }
      },
      ...
      async doAU(){
        ...
        if(!this.form.id){
          await UserAdminService.doAddEmpUser(this.form);
        } else {
          await UserAdminService.doChangeInfor(this.form);
        }
