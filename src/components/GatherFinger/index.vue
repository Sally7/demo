<template>
  <div class="coupon">
      <el-dialog title="指纹采集"  width="440px" :visible="show" :before-close="handleClose" style="text-align: center">
      ...
      eventBus.$on('onGatherFinger',function(data){
        ...
        switch(data.status)
        {
          case "1":that.message = "执行成功";
            ...
            that.gatherSatus = 1;
            that.fingerCode = data.fingerCode;
            that.doGather();
            break;
    ...
    methods: {
      ...
      async doGather(){
        if(this.gatherSatus == 0) {
          ...
          res.fingerCode = this.fingerCode;
